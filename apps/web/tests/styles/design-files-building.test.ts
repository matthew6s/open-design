...
const emptyStateCss = readFileSync(
  new URL('../../src/components/design-files/DesignFilesEmptyState.module.css', import.meta.url),
  'utf8',
);
...
describe('run step feed styles', () => {
  ...
  it("keeps the empty state text block click-through", () => {
    expect(cssDeclarations(emptyStateCss, '.center')).toMatch(
      /(?:^|[;\n])\s*pointer-events:\s*none\s*;/,
    );
  });
});
