...
- **在跑的工具输出默认是否展开**。`ToolRow` 把 `terminal` 渲染在**默认收起**的 `Foldable` 里,而「执行 `<命令>`」那种没有人类标题的变体**连 terminal 盒子都不渲染**。也就是说 vela 转发过来的实时 stdout 目前**看不见**。三个方向:①在跑时默认展开 ②行上只露最后一两行(tail)③折叠头上显示"已输出 N 行"且数字在涨。
- **`chat.forkedConversationTitle` 仍叫「{title} 分叉」**。它是持久化的会话**标题**不是控件标签,T46 没覆盖到,但属同一词族。
