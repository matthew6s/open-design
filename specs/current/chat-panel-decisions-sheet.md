...
| **T49** | **失败的文件行也要能下拉展开看报错原文**(与失败命令行同一个待遇) | 产品口述 2026-09-03:「能下拉展开吗?像这样」。⚠️ **有意偏离稿子** —— 稿子 `:917` 的文件类失败是单行 + 「失败」按钮。落地时 call site 必须写明这是口述裁决而非照稿 |
...
- **失败的文件行要不要也能下拉展开**。产品 2026-09-03 口述「能下拉展开吗?像这样」(指着失败命令行那个折叠块),方向已定,但**稿子没画过**这一态 —— `body-components.html:917` 的文件类失败是单行 + 一颗「失败」按钮,只有 `:1018-1019` 的命令类失败是 `<details class="fold is-fail" open>`。落地中,归 T49。
