...
printf '{"capabilities":{"contentUpdater":"standalone-v4","sharedInstance":"fixture-v3","shellUpdater":"fixture-v3"},"carrierLock":{"file":"carrier.lock","sha256":"%s"},"contracts":{"file":"contract/index.json","sha256":"%s"},"fixtureLifecycle":{"entrypoint":"runtime/fixture-lifecycle.mjs","sha256":"%s"},"fixtureShellUpdater":{"entrypoint":"runtime/fixture-shell-updater.mjs","sha256":"%s"},"fossil":{"entrypoint":"runtime/fossil.mjs","sha256":"%s"},"releaseDocuments":{"content":{"file":"release/content-metadata.json","sha256":"%s"}},"runtime":{"executable":"%s","name":"node","sha256":"%s","version":"%s"},"schemaVersion":1,"seed":{"closure":{"file":"seed/closure.mjs","sha256":"%s"},"standaloneLauncher":{"file":"runtime/standalone/index.mjs","sha256":"%s"}},"shell":{"buildHash":"%s","type":"terminal","version":"%s"},"shellFiles":{"ps1":{"install":{"file":"ps1/install.ps1","sha256":"%s"},"terminal":{"file":"ps1/terminal.ps1","sha256":"%s"}},"sh":{"install":{"file":"sh/install.sh","sha256":"%s"},"terminal":{"file":"sh/terminal.sh","sha256":"%s"}}},"standalone":{"entrypoint":"runtime/standalone/index.mjs","sha256":"%s"},"target":"%s","trust":{"file":"trust/keys.json","sha256":"%s"}}\n' \
  "$carrier_lock_sha" "$contract_index_sha" "$fixture_lifecycle_sha" "$fixture_shell_updater_sha" "$fossil_sha" "$content_sha" "$node_executable" "$node_sha" "$node_version" "$closure_sha" "$standalone_sha" "$shell_build_hash" "$shell_version" "$ps_install_sha" "$ps_terminal_sha" "$sh_install_sha" "$sh_terminal_sha" "$standalone_sha" "$target" "$trust_sha" > "$root/install-manifest.json"
